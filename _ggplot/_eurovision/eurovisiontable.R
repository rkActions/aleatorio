...
circle_gr <- grid.circle(gp = gpar(col = NA, fill = radialGradient(colours = c("#ffffff", NA))))


circle_gr <- grid.circle(gp = gpar(col = NA, fill = radialGradient(colours = c("#000000", NA))))
...
ggplot(euro22) +
  ...
  geom_text(aes(x, y, label = str_pad(rank, 2, pad = "0")), size = 6, color = "white", family = "mono", fontface = "bold") +
  # Flag
  as_reference(
  ...
  # Country
  geom_text(aes(x + 1.8, y, label = artist_country), size = 5.5, color = "white", hjust = 0, family = "mono") +
  # Points
  geom_text(aes(x + 7.2, y, label = total_points), size = 5.5, color = "white", hjust = 1, family = "mono") +
  labs(
  ...
  theme(
    ...
    plot.title = element_text(color = "grey98", hjust = 0.5, family = "mono", face = "bold", size = 40, margin = margin(50, 0, -60, 0)),
    plot.caption = element_text(size = 12, color = "grey98", hjust = 0.5, family = "mono", margin = margin(-20, 0, 20, 0))
  )
